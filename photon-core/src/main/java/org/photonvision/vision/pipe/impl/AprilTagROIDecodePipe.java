...
public class AprilTagROIDecodePipe
    ...
    public static RotatedRect expandBbox(
        ...
        int x = (int) Math.round(Math.max(0.0, Math.min(left, imageWidth - newWidth)));
        int y = (int) Math.round(Math.max(0.0, Math.min(top, imageHeight - newHeight)));

        return fromIntRect(x, y, newWidth, newHeight, bbox.angle);
    }

    /**
     * Builds a RotatedRect whose axis-aligned boundingRect matches the integer ROI. OpenCV's
     * boundingRect includes both edge pixels, so the continuous rectangle is one pixel smaller.
     */
